...
import io.github.tanice.terraCraft.api.items.components.TerraCustomNameComponent;
...
public class CustomNameComponent implements TerraCustomNameComponent {
    ...
    public CustomNameComponent(@Nonnull String name) {
    ...
    public void apply(TerraBaseItem item) {
        if (ServerVersion.isAfterOrEq(MinecraftVersions.v1_20_5)) {
            ...
            NBT.modify(item.getBukkitItem(), nbt -> {
                nbt.getOrCreateCompound(TAG_KEY).getOrCreateCompound("display").setString("Name", MiniMessageUtil.toNBTJson(name));
            });
        }
    }
}
