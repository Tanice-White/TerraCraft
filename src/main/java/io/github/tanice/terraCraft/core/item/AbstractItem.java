...
public abstract class AbstractItem implements TerraBaseItem {
    ...
    public AbstractItem(ConfigurationSection cfg) {
        ...
        for (TerraBaseComponent component : vanillaComponents) component.cover(bukkitItem);
    }
    ...
    private void processComponents(ConfigurationSection cfg) {
        ...
        if (cfg.isSet("!attributes")) AttributeModifiersComponent.remove(bukkitItem);
        if (cfg.isSet("shield")) {
            sub = cfg.getConfigurationSection("shield");
            if (sub != null) vanillaComponents.add(new BlocksAttacksComponent(sub));
        }
        if (cfg.isSet("!shield")) BlocksAttacksComponent.remove(bukkitItem);
        ...
        if (cfg.isSet("!break_sound")) BreakSoundComponent.remove(bukkitItem);
        ...
        if (cfg.isSet("!consumable")) ConsumableComponent.remove(bukkitItem);
        ...
        if (cfg.isSet("!ori_durability")) DamageComponent.remove(bukkitItem);
        if (cfg.isSet("resistant")) {
            vanillaComponents.add(new DamageResistantComponent(TerraNamespaceKey.from(cfg.getString("resistant"))));
        }
        if (cfg.isSet("!resistant")) DamageResistantComponent.remove(bukkitItem);
        ...
        if (cfg.isSet("!use_cooldown")) UseCooldownComponent.remove(bukkitItem);
        ...
        if (cfg.isSet("!use_remainder")) UseRemainderComponent.remove(bukkitItem);
        if (cfg.isSet("weapon")) {
            sub = cfg.getConfigurationSection("weapon");
            if (sub != null) vanillaComponents.add(new WeaponComponent(sub));
        }
        if (cfg.isSet("!weapon")) WeaponComponent.remove(bukkitItem);
    }
}
